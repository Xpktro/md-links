...
const extractContent = file => new Promise(resolve => (0, _fs.readFile)(file, (err, contents) => resolve(contents.toString())));
...
const extractLinks = file => {
  const linkRegex = /\[([^\[\]]+)\]\(([^\(\)]+)\)/g;
  return extractContent(file).then(contents => [].concat(...(contents || '').split('\n').map((line, lineNumber) => {
